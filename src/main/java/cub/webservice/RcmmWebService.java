...
public class RcmmWebService {
	...
	@EJB
	private static UdDataScopeMasterFacade udDataScopeMasterFacade;
	...
	public RcmmResponseObject getResponsePost(RcmmRequestObject request) {
		...
		List<RuleList> ruleList_T = new ArrayList<>();
		String returnCode = "";
		String resReturnCode = "";
		if (channelCode.compareTo(BigDecimal.ZERO) == 0) {
			returnCode = "9999";
			rtnMessage = "找不到對應APID:" + request.getApid();
		} else {
			orderInfo = genOrderInfo(request, channelCode);
			// 3.GEN_APPLIED_RULE 產生下單適用法規檔
			ruleList_T = ruleListFacade.joinRuleList(request, channelCode);
			// RTN_CHECK_RESULT
			returnCode = rtnCheckResult(ruleList_T, orderInfo);
		}
	...
	private OrderInfo genOrderInfo(RcmmRequestObject request, BigDecimal channelCode) {
		...
		String orderNo = StringUtils.isBlank(request.getOrderNo())
				? StringUtils.isBlank(request.getMasterOrderNo()) ? "" : request.getMasterOrderNo()
				: request.getOrderNo();
		if (StringUtils.isBlank(orderNo)) {
			orderNo = "TR" + cal.get(Calendar.YEAR) + orderInfoSeq.substring(1, orderInfoSeq.length());
		}
		orderInfo.setOrderNo(orderNo);
		orderInfo.setMasterOrderNo(orderNo);
	...
	private static BigDecimal rtnScopeValue(String scopeCode, OrderInfo orderInfo, RuleList rl) {
		if (scopeCode.startsWith("DF")) {
			UdDataScopeMaster ma = udDataScopeMasterFacade.find(scopeCode);
			String apiCode = ma.getApiCode();
